"""
Configuration manager for persistent download path storage.

This module handles storing the user's preferred download directory using a JSON
configuration file located in the project root. Provides both setter functionality
(when a path is provided) and getter functionality (when called without arguments).
"""
...
def configuring_path(path: str) -> str:
    ...
    Acts as both a setter and getter for the download directory:
    ...
    Args:
        path: Directory path to save files. If empty/falsy, acts as getter returning
              current configuration. If provided, acts as setter saving the path.

    Returns:
        Status message indicating either:
    ...
    Note:
        The configuration file is stored as 'config.json' in the project root directory
        (one level above the 'modules' folder). The file uses UTF-8 encoding and
        pretty-printed JSON formatting for human readability.
    """
    ...
    if path:
        ...
        config = {
            "path": str(path),  # Convert to string for JSON serialization
        }
